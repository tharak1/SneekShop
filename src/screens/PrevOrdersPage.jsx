...
import Button from '@mui/material/Button';
import Navbar from '../components/Navbar';
import { fetchUserOrders, getUserOrders } from '../redux/userOrdersSlice';
import { setUser } from '../redux/userSlice';

const PrevOrdersPage = () => {
  const dispatch = useDispatch();
  const temp = useSelector(setUser);
  const { orders, status, error } = useSelector(getUserOrders);

  useEffect(() => {
    const userId = temp.userData.uid; // Replace with actual user ID
  ...
  return (
    <>
      ...
      <div className="w-full mt-24 flex flex-col items-center justify-center">
        ...
        {status === 'succeeded' && orders.length === 0 && <p>No previous orders found.</p>}

        {status === 'succeeded' && orders.map((order, index) => (
          <div key={index} className="flex md:flex-row justify-start items-start md:items-center border border-gray-200 w-2/3 my-2">
            <div className="w-40 md:w-32">
              <img className="hidden md:block" src={order.image} alt={order.name} />
              <img className="md:hidden" src={order.image} alt={order.name} />
            </div>
            <div className="flex justify-start md:justify-between items-start md:items-center flex-col md:flex-row w-full p-4 md:px-8">
              <div className="flex flex-col md:flex-shrink-0 justify-start items-start">
                <h3 className="text-lg md:text-xl w-full font-semibold leading-6 md:leading-5 text-gray-800">{order.name}</h3>
                <div className="flex flex-row justify-start space-x-4 md:space-x-6 items-start mt-4">
                  <p className="text-sm leading-none text-gray-600">
